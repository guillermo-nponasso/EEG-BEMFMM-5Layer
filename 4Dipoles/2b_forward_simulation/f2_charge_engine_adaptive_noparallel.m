...
%% define options
numThreads = 1 % used for compatibility
...
for adapt = 1:maxRefSteps
    ...
    %   Compute accurate integration for electric field/electric potential on all neighbor facets
        ...
        warning off
        tic
        EC                  = meshneighborints_En_noparallel(P, t, normals, Area, Center, RnumberE, ineighborE, numThreads);
        Neighbor.EnTime = toc;
        tic
        [PC, integralpd]    = meshneighborints_Pn_noparallel(P, t, normals, Area, Center, RnumberP, ineighborP, Indicator, numThreads);
        Neighbor.Ptime = toc;
        disp(Neighbor)
        warning on
