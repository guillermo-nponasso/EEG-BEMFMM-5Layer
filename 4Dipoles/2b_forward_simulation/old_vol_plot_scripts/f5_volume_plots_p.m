...
R = 5;         %  Distance threshold (dimensionless, scaled to triangle size) that determines whether to use precise integration
Esec            = zeros(Ms*Ms, 3);
[Epri, ~]   = bemf3_inc_field_electric_plain(strdipolePplus, strdipolePminus, strdipolesig, strdipoleCurrent, pointsXZ*1e-3);
...
R = 5;         %  Distance threshold (dimensionless, scaled to triangle size) that determines whether to use precise integration
Esec           = zeros(Ms*Ms, 3);
% [Epri, ~]   = bemf3_inc_field_electric_plain(strdipolePplus, strdipolePminus, strdipolesig, strdipoleCurrent, pointsYZ*1e-3);
Esec(in, :)    = bemf5_volume_field_potential(pointsYZ(in, :)*1e-3, c, P, t, Center, Area, normals, R, planeABCD);
Etotal         = Esec;   
fieldPlaneTime = toc   
