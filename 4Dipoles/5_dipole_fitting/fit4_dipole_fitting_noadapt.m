%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% fit4_dipole_fitting_noadapt.m                                       %%%
%%% Fit a dipole for the selected model and raw data. No adaptive sol.  %%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% load non-adaptive solution and create new electrode data
...
run('e0_set_electrodes_noamr.m');
run('e1_prepare_raw_data.m');
cd ..;
cd(fullfile('5_dipole_fitting')); % back to present folder

fprintf("PROCESSING NON-AMR DIPOLE FITTING FOR DIPOLE %s. PLEASE WAIT..",dipole_name);
disp(".......................................................");
%% create orig dipole info
orig_dip = load(fullfile(patient_path,'dipoles',dipole_name, ...
    strcat(patno,'_',dipole_name,'_data.mat')));

dip0.dip.pos = orig_dip.strdipolemcenter;
...
dist_dip = 1e3*norm(dip.dip.pos-dip0.dip.pos); % display error in mm
