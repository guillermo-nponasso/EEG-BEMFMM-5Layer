...
%%  Setup parallel port
numThreads      = 30;      %   number of cores to be used
tic
ppool           = gcp('nocreate');
if isempty(ppool)
    tic
...
R               = 0.01;         %   For Gauss selective
X = 1e3*ClusterCenter(1);   % mm
Y = 1e3*ClusterCenter(2);   % mm
Z = 1e3*ClusterCenter(3);   % mm

%% Load compartment names and assign conductivities
index_name  = 'tissue_index.txt';
condambient = 0;
mesh_path=fullfile('../data');
[name, tissue, cond, enclosingTissueIdx] = tissue_index_read(index_name, mesh_path);
...
D           = Center - repmat(ClusterCenter, size(Center, 1), 1);
DIST        = sqrt(sum(D.*D, 2));
